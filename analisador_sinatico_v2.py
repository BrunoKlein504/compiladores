...
import ap1.analisador_lexico as AL
...
class Analisador_Sintatico:
    ...
    def consome(self, atomo):
        if atomo != self.lookahead.tipo:
            # if atomo != self.lookahead.operador:
            ...
            if self.lookahead.valor: # Usando truthy caso o valor decimal seja maior que zero
                print(f'Linha: {self.lookahead.linha} - Atomo: {AL.atomo_msg[self.lookahead.tipo]} \tLexema: {self.lookahead.lexema}   valor: {self.lookahead.valor}')
            else:
                print(f'Linha: {self.lookahead.linha} - Atomo: {AL.atomo_msg[self.lookahead.tipo]} \tLexema: {self.lookahead.lexema}')
            self.lookahead = self.lexico.proximo_atomo()
    ...
    def programa(self):
       ...
       if self.lookahead.tipo == AL.PROGRAM:
           ...
           if self.lookahead.tipo == AL.ABRE_PARENT:
               ...
               if self.lookahead.tipo == AL.FECHA_PARENT:
           ...
           if self.lookahead.tipo == AL.PONTO:
    ...
    def declaracao(self):
        ...
        self.lista_de_identi()
        if self.lookahead.tipo == AL.DOIS_PONTOS:
    ...
    def atribuicao(self):
        ...
        if self.lookahead.tipo == AL.IDENTIFICADOR:
            ...
            if self.lookahead.tipo == AL.ATRIB:
...
#                   [else <comando>]
        if self.lookahead.tipo == AL.IF:
            self.consome(AL.IF)

            self.expressao()

            if self.lookahead.tipo == AL.THEN:
    ...
    def comando_while(self):
        ...
        if self.lookahead.tipo == AL.WHILE:
            self.consome(AL.WHILE)

            self.expressao()

            if self.lookahead.tipo == AL.DO:
                self.consome(AL.DO)
    ...
    def comando_entrada(self):
        ...
        if self.lookahead.tipo == AL.READ:
    ...
    def comando_saida(self):
        ...
        if self.lookahead.tipo == AL.WRITE:
...
try:
    ...
    print(f'{linhas} linhas analisadas, programa sintaticamente correto.')